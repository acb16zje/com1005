/**
 * MStringVector.java a vector of Mstring with facilities for pattern matching Created: Jan 2013
 *
 * @author phil green
 * @version 4
 */
...
public class MStringVector extends Object {
    ...
    public MStringVector(ArrayList<MString> vin) {
        v = vin;
    }
    ...
    public MStringVector(String[] slis) {
        v = new ArrayList<MString>();
        for (int j = 0; j < slis.length; j++) {
            v.add(new MString(slis[j]));
        }
    }

    // accessors
    public ArrayList<MString> getV() {
        return v;
    }

    public void setV(ArrayList<MString> vv) {
        v = vv;
    }
    ...
    public ArrayList<MString> getRemVec() {
        return remVec;
    }
 ...
 */
    ...
    public void addMString(MString s) {
        v.add(s);
    }
    ...
    public boolean matchAll(String s) {
        ...
        for (MString nextV : v) {
            ...
            if (result1) {
                ...
                cloneV.remove(p);
                MatchDetails md = new MatchDetails(p.getContext(), p.getStr(),

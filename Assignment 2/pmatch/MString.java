/**
 * MString.java String as Matching pattern Can have variables - starting with ? - in either p or d
 * Created: Jan 2013
 *
 * @author phil green
 * @version 3
...
public class MString extends Object {
    ...
    public HashMap getpCon() {
        return pCon;
    }

    public HashMap getdCon() {
        return dCon;
    }
        ...
        if (pTok.length != dTok.length) {
            ...
            while (j < pTok.length && result) {
                ...
                if (!nextP.equals(nextD)) {
                    ...
                    } else if (pVar(nextD)) { //got an mvar in d
    ...
    public String mSubst(HashMap c) {
        ...
        while (j < pTok.length) {
            ...
            if (res == null) {
                newPatt = newPatt.concat(np);
            } else {
                newPatt = newPatt.concat(res);
            }

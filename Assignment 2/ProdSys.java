/**
 * ProdSys.java - class for production system engine 2013 Version
 */
...
public class ProdSys {
    ...
    public ArrayList<String> RunPS(ArrayList<String> s) { //is given the initial stm
        ...
        while (prodFired) { // continue until no production fires
            ...
            while (i.hasNext() && !prodFired) { //iterate over the prods until one fires or run out
                ...
                MStringVector antes = new MStringVector(
                    r.getAntes()); //its antecedants as an MStringVector
                ArrayList<MatchDetails> partials = new ArrayList<MatchDetails>(); // partial matches
                partials.add(new MatchDetails(new HashMap(), "",
                    antes)); // initiallly 1 partial with all the antes & an empty context
    ...
    private void fire_prodn(Prodn p, HashMap c) {
        ...
        MStringVector addsm = new MStringVector(
            p.getAdds());//make MStringVectors for the prods adds, dels & remarks
        ...
        scr.println("Deletions:");
        for (String d : dels) {
            scr.println("\t" + d);
        }

        /*scr.println("Additions:");
        for (String a : adds) {
            scr.println("\t" + a);
        }*/
